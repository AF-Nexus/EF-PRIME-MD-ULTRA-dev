...
case 'add': {
    if (!sender.endsWith('@g.us')) return await socket.sendMessage(sender, { text: '❌ This command is only for groups!' });
...
case 'setgroupname': {
    ...
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!' });
    if (!args[0]) return await socket.sendMessage(sender, { text: `❌ Usage: ${config.PREFIX}setgroupname <new name>` });
...
case 'setbotname': {
    const isOwner = number.replace(/[^0-9]/g, '') === config.OWNER_NUMBER.replace(/[^0-9]/g, '');
    if (!isOwner) return await socket.sendMessage(sender, { text: '❌ Only owner can use this command!' });
    if (!args[0]) return await socket.sendMessage(sender, { text: `❌ Usage: ${config.PREFIX}setbotname <new name>` });
