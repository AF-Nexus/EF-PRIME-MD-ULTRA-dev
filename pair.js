...
case 'ping': {     
    ...
    let ping = await socket.sendMessage(sender, { text: '*_Pinging.. ._* 🐥', quoted: msg });
...
case 'wyr': {
    ...
    try {
        ...
        await socket.sendMessage(sender, { text: loadingMsg, quoted: msg });
...
case 'video': {
    try {
        ...
        const loadMsg = await socket.sendMessage(sender, { text: loadingMsg, quoted:  msg });
...
// ==================== ANTILINK COMMAND ====================
case 'antilink': {
    if (!sender.endsWith('@g.us')) return await socket.sendMessage(sender, { text: '❌ This command is only for groups! ', quoted: msg });
    ...
    if (!isBotAdmin) return await socket. sendMessage(sender, { text:  '❌ Bot must be admin!', quoted: msg });
    if (!isAdmin) return await socket. sendMessage(sender, { text:  '❌ Only admins can use this! ', quoted: msg });
    ...
    await socket.sendMessage(sender, {
        text: `${icon} Antilink ${status}\n\nBot will now ${action === 'on' ? 'remove members sending links' : 'allow links'}! `,
        quoted: msg
    });
    break;
}
...
case 'add': {
    ...
    if (!isBotAdmin) return await socket. sendMessage(sender, { text:  '❌ Bot must be admin to add members!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted: msg });

    const mentionedJid = msg.message.extendedTextMessage?.contextInfo?. mentionedJid || [];
    const users = mentionedJid.length > 0 ? mentionedJid : [args[0] + '@s.whatsapp.net'];
    
    let adding = await socket.sendMessage(sender, { text: '*_Adding members.. ._* ⏳', quoted: msg });
...
case 'kick': {
    ...
    if (!isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to remove members!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted: msg });

    const mentionedJid = msg.message. extendedTextMessage?.contextInfo?.mentionedJid || [];
    const users = mentionedJid.length > 0 ? mentionedJid : [args[0] + '@s. whatsapp.net'];
    
    let removing = await socket.sendMessage(sender, { text: '*_Removing members..._* ⏳', quoted: msg });
...
case 'promote': {
    ...
    if (!isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to promote members!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted:  msg });

    const mentionedJid = msg.message.extendedTextMessage?.contextInfo?. mentionedJid || [];
    const users = mentionedJid.length > 0 ?  mentionedJid : [args[0] + '@s.whatsapp.net'];
    
    let promoting = await socket.sendMessage(sender, { text: '*_Promoting member.. ._* ⏳', quoted: msg });
...
case 'demote': {
    ...
    if (!isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to demote members!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted: msg });

    const mentionedJid = msg.message. extendedTextMessage?.contextInfo?.mentionedJid || [];
    const users = mentionedJid.length > 0 ? mentionedJid : [args[0] + '@s. whatsapp.net'];
    
    let demoting = await socket.sendMessage(sender, { text: '*_Demoting admin..._* ⏳', quoted: msg });
...
case 'groupinfo': {
    ...
    let fetching = await socket.sendMessage(sender, { text: '*_Fetching group info..._* ⏳', quoted: msg });
...
case 'setgroupname': {
    ...
    if (!isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to change group name!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted: msg });
    if (! args[0]) return await socket.sendMessage(sender, { text: `❌ Usage: ${config.PREFIX}setgroupname <new name>`, quoted: msg });

    const newName = args. join(' ');
    let updating = await socket.sendMessage(sender, { text: '*_Updating group name..._* ⏳', quoted: msg });
...
case 'setdesc': {
    ...
    if (!isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to change description!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted:  msg });
    if (!args[0]) return await socket.sendMessage(sender, { text: `❌ Usage: ${config.PREFIX}setdesc <new description>`, quoted: msg });

    const newDesc = args.join(' ');
    let updating = await socket.sendMessage(sender, { text: '*_Updating description..._* ⏳', quoted: msg });
...
case 'lock': {
    ...
    if (!isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to lock group!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted:  msg });

    let locking = await socket.sendMessage(sender, { text: '*_Locking group..._* 🔒', quoted: msg });
...
case 'unlock': {
    ...
    if (!isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to unlock group!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted: msg });

    let unlocking = await socket.sendMessage(sender, { text: '*_Unlocking group..._* 🔓', quoted: msg });
...
case 'leave': {
    ...
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can make bot leave!', quoted: msg });
...
case 'invite': {
    ...
    if (!isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to get invite link!', quoted: msg });

    let fetching = await socket.sendMessage(sender, { text: '*_Fetching invite link..._* ⏳', quoted: msg });
...
case 'revoke': {
    ...
    if (! isBotAdmin) return await socket.sendMessage(sender, { text: '❌ Bot must be admin to revoke invite!', quoted: msg });
    if (!isAdmin) return await socket.sendMessage(sender, { text: '❌ Only admins can use this command!', quoted:  msg });

    let revoking = await socket.sendMessage(sender, { text: '*_Revoking invite link.. ._* ⏳', quoted: msg });
...
case 'block': {
    ...
    let blocking = await socket.sendMessage(sender, { text: '*_Blocking user..._* ⏳', quoted: msg });
...
case 'unblock': {
    ...
    const mentionedJid = msg.message. extendedTextMessage?.contextInfo?.mentionedJid || [];
    const user = mentionedJid[0] || args[0] + '@s.whatsapp.net';
    let unblocking = await socket.sendMessage(sender, { text: '*_Unblocking user..._* ⏳', quoted: msg });
...
case 'setbio': {
    const isOwner = number.replace(/[^0-9]/g, '') === config.OWNER_NUMBER.replace(/[^0-9]/g, '');
    if (!isOwner) return await socket.sendMessage(sender, { text: '❌ Only owner can use this command!', quoted: msg });
    if (!args[0]) return await socket.sendMessage(sender, { text: `❌ Usage: ${config.PREFIX}setbio <new bio>`, quoted: msg });

    const newBio = args. join(' ');
    let updating = await socket.sendMessage(sender, { text: '*_Updating bio..._* ⏳', quoted: msg });
...
case 'setbotname': {
    const isOwner = number.replace(/[^0-9]/g, '') === config.OWNER_NUMBER.replace(/[^0-9]/g, '');
    if (!isOwner) return await socket.sendMessage(sender, { text: '❌ Only owner can use this command!', quoted: msg });
    if (!args[0]) return await socket.sendMessage(sender, { text: `❌ Usage: ${config.PREFIX}setbotname <new name>`, quoted: msg });

    const newName = args.join(' ');
    let updating = await socket.sendMessage(sender, { text: '*_Updating name..._* ⏳', quoted: msg });
