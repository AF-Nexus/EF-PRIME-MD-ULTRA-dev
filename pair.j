...
├ ${config.PREFIX}play <query> - Play song
...
├ ${config.PREFIX}setgroupname <name> - Change name
├ ${config.PREFIX}setdesc <desc> - Change description
├ ${config.PREFIX}lock - Lock group (admin only)
├ ${config.PREFIX}unlock - Unlock group
├ ${config.PREFIX}leave - Bot leaves group
├ ${config.PREFIX}tagall <message> - Tag all
...
├ ${config.PREFIX}setbio <text> - Update bio
...
│ 💡 Tip: Use ${config.PREFIX}help <cmd>
├──────────────────
│ ${config.BOT_FOOTER}
╰──────────────────❒`;
...
╰──────────❒`;
                ...
                case 'add': {
                    ...
                    try {
                        ...
                        try {
                            ...
                            await socket.sendMessage(sender, {
                                text:  `✅ *MEMBERS ADDED*\n\n${users.length} member(s) successfully added to the group!`,
                                edit: adding.key
                            });
                        } catch (err) {
                            await socket.sendMessage(sender, {
                                text: designTemplates.errorDesign('Failed to add members!  They might have privacy settings.'),
                                edit: adding.key
                            });
                        }
                    } catch (error) {
                ...
                case 'kick': {
                    ...
                    try {
                        ...
                        try {
                            ...
                            await socket.sendMessage(sender, {
                                text:  `✅ *MEMBERS REMOVED*\n\n${users.length} member(s) successfully removed from the group!`,
                                edit: removing.key
                            });
                        } catch (err) {
                            await socket.sendMessage(sender, {
                ...
                case 'promote': {
                    ...
                    try {
                        ...
                        if (!isBotAdmin) {
                            return await socket.sendMessage(sender, { 
                                text: designTemplates.errorDesign('Bot must be admin to promote members!'),
                                quoted: msg 
                            });
                        }
                        if (!isUserAdmin) {
                            return await socket. sendMessage(sender, { 
                                text: designTemplates. errorDesign('Only admins can promote members!'),
                                quoted: msg 
                            });
                        }
                        ...
                        try {
                            ...
                            await socket.sendMessage(sender, {
                                text: `✅ *MEMBERS PROMOTED*\n\n${users. length} member(s) successfully promoted to admin!`,
                                edit: promoting.key
                            });
                        } catch (err) {
                            await socket.sendMessage(sender, {
                ...
                case 'demote': {
                    ...
                    try {
                        ...
                        if (!isBotAdmin) {
                            return await socket.sendMessage(sender, { 
                                text:  designTemplates.errorDesign('Bot must be admin to demote members!'),
                                quoted: msg 
                            });
                        }
                        if (!isUserAdmin) {
                            return await socket.sendMessage(sender, { 
                                text: designTemplates.errorDesign('Only admins can demote members!'),
                                quoted: msg 
                            });
                        }
                        ...
                        try {
                            ...
                            await socket.sendMessage(sender, {
                                text:  `✅ *ADMINS DEMOTED*\n\n${users.length} admin(s) successfully demoted to member!`,
                                edit: demoting.key
                            });
                        } catch (err) {
                            await socket.sendMessage(sender, {
...
async function EmpirePair(number, res) {
    ...
    try {
        ...
        const logger = pino({ level: process.env.NODE_ENV === 'production' ?  'silent' : 'error' });
        ...
        socket.ev.on('creds.update', async () => {
            try {
                // Save credentials to local session folder first
                await saveCreds();
                
                // Wait for file to be written to disk
                await delay(500);
                
                // Build the full path to creds file
                const credspath = path. join(sessionPath, 'creds.json');
                
                // Check if file exists before reading
                if (!fs.existsSync(credspath)) {
                    console.warn(`⚠️ Creds file not found at ${credspath}, retrying...`);
                    await delay(1000);
                    
                    if (!fs.existsSync(credspath)) {
                        console.error(`❌ Creds file still missing after retry at ${credspath}`);
                        return;
                    }
                }
                ...
                try {
                    ...
                    sha = data.sha;  // File exists on GitHub, we'll update it
                } catch (error) {
                    // File doesn't exist on GitHub yet (first time), that's fine
                    console.log(`ℹ️ First time uploading creds for ${sanitizedNumber}`);
                }
                ...
                console.log(`✅ Successfully synced credentials to GitHub for ${sanitizedNumber}`);
            } catch (error) {
                console.error(`❌ Failed to sync credentials to GitHub: ${error.message}`);
