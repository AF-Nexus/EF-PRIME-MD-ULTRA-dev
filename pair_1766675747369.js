...
╰──────────────❒`,
                ...
                case 'viewonce': {
                    try {
                        ...
                        if (!quotedMsg) return await socket.sendMessage(sender, { text: designTemplates.errorDesign('Please reply to a view once message!') });
                        ...
                        if (!viewOnceMsg) return await socket.sendMessage(sender, { text: designTemplates.errorDesign('This is not a view once message!') });
                        ...
                        const mediaBuffer = await downloadMediaMessage(
                            { key: msg.message.extendedTextMessage.contextInfo, message: viewOnceMsg },
                            'buffer',
                            {},
                        ...
                        if (type === 'image') {
                            await socket.sendMessage(sender, { image: mediaBuffer, caption: `✅ *View Once Extracted*\n\nType: Image\n${config.BOT_FOOTER}`, quoted: msg });
                        } else if (type === 'video') {
                            await socket.sendMessage(sender, { video: mediaBuffer, caption: `✅ *View Once Extracted*\n\nType: Video\n${config.BOT_FOOTER}`, quoted: msg });
                        }
                    } catch (error) {
                        console.error('ViewOnce command error:', error);
                        await socket.sendMessage(sender, { text: designTemplates.errorDesign('Failed to extract view once message!') });
                    }
                    break;
                }
...
╰──────────❒`;
                ...
                case 'spotify': {
                    try {
                        ...
                        const res = await axios.get(
                            `https://veron-apis.zone.id/downloader/spotify?query=${encodeURIComponent(q)}&type=json`
                        );

                        const data = res.data?.result;
                        if (!data || !data.downloadInfo) {
                             await socket.sendMessage(sender, {
                                text: designTemplates.errorDesign('Song not found on Spotify!'),
                                edit: loadingMsg.key
                            });
                            return;
                        }

                        const { metadata, downloadInfo } = data;
...
╰──────────❒`;
                        ...
                        const downloadUrl = `https://veron-apis.zone.id${downloadInfo.directDownload}`;
                        ...
                        await socket.sendMessage(sender, {
                            text: designTemplates.errorDesign('Failed to fetch from Spotify. Please try again!'),
                            quoted: msg
                        });
                    }
                    break;
                ...
                case 'sticker':  {
                    try {
                        ...
                        try {
                            ...
                            // Remove message confirmation as per user request
                        } catch (err) {
